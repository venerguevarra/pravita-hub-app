...
```typescript
import { TenantMembershipApi, Configuration } from "./api";
...
const { status, data } = await apiInstance.add(tenantId, userId);
```
...
```typescript
import { TenantMembershipApi, Configuration, PageDto } from "./api";
...
const { status, data } = await apiInstance.getTenantUsers(tenantId, pageRequest);
```
...
const { status, data } = await apiInstance.remove(tenantId, userId, hardDelete);
```
