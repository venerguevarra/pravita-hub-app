...
const { status, data } = await apiInstance.getActiveUsers1(
    pageRequest
);
```
...
const { status, data } = await apiInstance.getAllUsers1(
    pageRequest
);
```
...
const { status, data } = await apiInstance.getUserBySystemId(
    systemId
);
```
